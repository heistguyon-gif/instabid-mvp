...
export default function PrivacyPage() {
  return <InfoPage eyebrow="Privacidade · Privacy" title="Coletar o mínimo. Explicar o necessário." lead="Este resumo descreve o piloto pago do Instabid e os dados necessários para operar o ranking.">
    <h2>Dados enviados</h2><p>Nome do projeto, perfil público, descrição, categoria, link de destino, e-mail de contato e valor do boost. No checkout Pix também são solicitados nome do pagador, CPF ou CNPJ e telefone para processamento e prevenção a fraude.</p>
    ...
    <h2>Pagamentos</h2><p>O Pix é processado pela BravoPay. A Instabid não armazena CPF/CNPJ ou telefone do pagador em seu banco; guarda apenas identificadores da cobrança, valor, status, código Pix e registros necessários para confirmação e auditoria.</p>
    ...
    <h2>English summary</h2><p>The pilot stores project data and payment references, while BravoPay processes Brazilian Pix payer data. Instabid does not retain payer tax IDs or phone numbers in its own database. A strictly necessary anonymous cookie is used to deduplicate clicks.</p>
  </InfoPage>;
}
