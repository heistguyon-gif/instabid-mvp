...
const fallbackBoards: Record<Market, BoardItem[]> = {
  br: [
    { id: 'partner-nexoflow', rank: 1, name: 'resolv.', handle: '@resolv.all', description: 'Soluções para negócios físicos e digitais: marketing, tecnologia e estrutura.', destinationUrl: 'https://instagram.com/resolv.all', category: 'Services', bid: '', clicks: '0', totalMinor: 0, currency: 'BRL', tone: 'sunset', placementType: 'launch_partner', imageUrl: '/resolv-all-avatar.jpg' },
    { id: 'partner-instabid', rank: 2, name: 'Instabid Brasil', handle: '@instabid.br', description: 'O ranking competitivo de perfis, marcas e projetos do Instagram.', destinationUrl: 'https://instagram.com/instabid.br', category: 'Brands', bid: '', clicks: '0', totalMinor: 0, currency: 'BRL', tone: 'violet', placementType: 'launch_partner', imageUrl: '/logo-emblem.png' },
  ],
  world: [
    ...
    const loadLeaderboard = () => fetch(`/api/leaderboard?market=${market}&period=${period}`, { signal: controller.signal, cache: 'no-store' })
      ...
      .then((payload) => {
        const items = payload.listings.map((item, index) => ({
          ...
          bid: Number(item.totalMinor) > 0 ? currency(Number(item.totalMinor), String(item.currency), market) : '', clicks: new Intl.NumberFormat(market === 'br' ? 'pt-BR' : 'en-US').format(Number(item.clicks)), tone: tones[index % tones.length], placementType: String(item.placementType ?? 'paid'), imageUrl: item.imageUrl ? String(item.imageUrl) : null,
        }));
          ...
          <div className="ranking-list">
            ...
            {visibleBoard.map((item) => {
              ...
              return <article aria-label={`Abrir detalhes de ${item.handle}`} className={`ranking-card ${item.rank <= 3 ? `podium podium-${item.rank}` : 'compact'} ${awaitingFirstBid ? 'awaiting-bid' : ''}`} key={item.id} onClick={() => setSelected(item)} tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelected(item); } }}>
                ...
                <button className="rank-challenge" onClick={(event) => { event.stopPropagation(); openJoin(challengeMinor, item); }} type="button">{awaitingFirstBid ? `Dar o primeiro lance · ${currency(challengeMinor, item.currency, market)}` : `Superar · ${currency(challengeMinor, item.currency, market)}`}<span>↗</span></button>
              </article>;
            })}
          </div>
        </section>
